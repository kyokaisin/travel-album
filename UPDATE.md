...
地图使用本地保存的 Leaflet 1.9.4 程序和 OpenStreetMap 在线瓦片；没有第三方字体、统计脚本或实时定位功能。版权信息保留在地图右下方。底图采用标准地理配色，页面与标记保持原来的绿色。
...
- 站点没有读取或展示用户设备 IP、邮箱、精确实时定位的代码，也没有分析统计。GitHub 托管及 OpenStreetMap 瓦片服务仍会处理网络 IP、浏览器信息；搜索非内置城市时 Open-Meteo 会收到搜索词及请求信息。
